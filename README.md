...
- **動態**：類似小小的部落格 / 動態牆，可以發文字、心情、一張照片，或貼一個
  YouTube 連結（會自動抓縮圖跟標題，做成像 LINE 那樣的小預覽卡片，
  點下去會開新分頁播放）。每篇貼文顯示發文者跟時間，另一半可以在下面留言，
  新貼文/新留言都會推播通知對方。貼文的照片一樣存在 Supabase Storage。
...
如果沒有設定這兩個環境變數，「回憶相簿」頁面會顯示提示訊息，上傳表單也會
被停用，不影響其他功能正常使用。

（留言裡的圖片上傳目前還是存在 Render 本機磁碟，重新部署後會消失；
如果想讓留言的圖片也永久保存，做法跟回憶相簿一樣，之後可以再跟我說我幫你加上去。）
...
- 目前是網頁版（PWA）；如果之後想要上架到 App Store / Google Play 的
  ...
  控制會比網頁版更完整（尤其是 iOS）。
- 留言裡的圖片上傳目前還是存在 Render 本機磁碟，重新部署後會消失；
  回憶相簿的照片已經用 Supabase Storage 解決了，如果想讓留言的圖片
  也一併搬過去，做法一樣，之後可以再跟我說。
